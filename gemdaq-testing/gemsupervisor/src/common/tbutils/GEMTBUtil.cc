...
void gem::supervisor::tbutils::GEMTBUtil::resetAction(toolbox::Event::Reference e)
  ...
  for (int i = 0; i < 24; ++i){
    confParams.bag.deviceName[i] = ""; // ensure that the selected chips are reset
    confParams.bag.connectedVFATs[i] = -999;
  }
  confParams.bag.n_connectedVFATs=0;
  
  for (int i = 0; i < 255; ++i)
    confParams.bag.channelName[i] = "";  // ensure that the selected channels are reset
